...
char* test_mat3_transform_vec(void){
    ...
    MU_ASSERT( "mat3_transform_vec0", fabs(t[0] - vec[2])<1e-6 );
    MU_ASSERT( "mat3_transform_vec1", fabs(t[1] - vec[1])<1e-6 );
    MU_ASSERT( "mat3_transform_vec2", fabs(t[2] - vec[0])<1e-6 );
    return 0;
 
}

char* test_determinant3(void){
    rmatrix a = { 0.30112582,  0.67285109,  0.37710869,  
                    0.43101797,  0.73377317,  0.60827196,  
                    0.42625684,  0.3349345 ,  0.9300366 };
    scalar d = -0.014623566951869022;
    scalar t;
...
char* test_inverse_mat3(void){
    rmatrix a = { 0.30112582,  0.67285109,  0.37710869,  
                    0.43101797,  0.73377317,  0.60827196,  
                    0.42625684,  0.3349345 ,  0.9300366 };
    rmatrix e = {-32.73514938,  34.15510178,  -9.06510735,
...
char* test_affine_transform_vec(void){
    ...
    MU_ASSERT("affinetransform_vec0",fabs(r[0] - 5.1)<1e-6);
    MU_ASSERT("affinetransform_vec1",fabs(r[1] - 4.2)<1e-6);
    MU_ASSERT("affinetransform_vec2",fabs(r[2] - 3.3)<1e-6);
    return 0;
}

char* test_mat3_prod(void){
    rmatrix a   = { 0.97300866,  0.53048923,  0.32333488,  
                    0.6900934 ,  0.44364110,  0.68283224,  
                    0.51383945,  0.87587622,  0.80127724 };
    rmatrix b   = { 0.06703767,  0.85926415,  0.38202935,  
                    0.68021704,  0.29180251,  0.38910406, 
                    0.59501276,  0.50158507,  0.07206758 };
    rmatrix c   = { 0.61846443,  1.15304949,  0.60143535, 
...
char* test_inverse_affine_mat(void){
    vector a = {4,5,6};
    afmatrix m = { 0,0,1,  1,0,0, 0,1,0, 0.1, 0.2, 0.3 };
    vector r;
    afmatrix im;
    vector ir;
    int i;
